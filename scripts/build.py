...
{seo.head(current)}
<link rel="icon" href="assets/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="assets/site.css"><script src="assets/site.js" defer></script>{'<script src="assets/allocation.js" defer></script>' if current=='next.html' else ''}</head><body>{header}<main id="main" class="wrap">{body}</main><footer class="site-footer wrap"><p>Energy Density Challenge · Open research · <a href="license.html" rel="license">MIT license</a></p><nav aria-label="Footer"><a href="review.html">Framing review</a><a href="guide.html#sources">Sources</a><a href="guide.html#safety">Safety</a><a href="{REPO}">GitHub</a><a href="resources.html">Share and reuse</a></nav>{share}</footer></body></html>\n'''
...
def allocation_body():
 content=read('templates/next.html')
 reviews=[]
 for part in re.split(r'^## ',read('docs/state-0002-review.md'),flags=re.M)[1:]:
  title,rest=part.split('\n',1)
  reviews.append('<details class="wide-detail review-pass"><summary>'+e(title)+'</summary>'+markdown(rest,'docs/state-0002-review.md')+'</details>')
 sources=[]
 for source in data('research/process/sources.json')['sources']:
  access='Abstract reviewed; full paper not evaluated' if source['verification']=='abstract_reviewed' else 'Page reviewed'
  sources.append('<article class="source-entry"><h3><a href="'+e(source['url'])+'">'+e(source['title'])+'</a></h3><p>'+e(source['supports'])+'</p><p><strong>Limit:</strong> '+e(source['does_not_support'])+'</p><p class="source-note">'+e(access)+' · '+e(source['accessed'])+'</p></article>')
 states=[]
 for entry in data('research/states/index.json')['states']:
  state=data(entry['manifest'])
  states.append('<p><a href="'+e(entry['manifest'])+'">'+e(entry['id'])+' manifest (JSON)</a><br><span class="state-digest">Merkle root: '+e(state['merkle_root'])+'</span></p>')
 return content.replace('@@ALLOCATION_REVIEW@@',''.join(reviews)).replace('@@PROCESS_SOURCES@@',''.join(sources)).replace('@@STATE_LINKS@@',''.join(states))
...
def render():
 ...
 product={'next.html':page('What should we learn next?',allocation_body(),'next.html')}
 ...
 license_body='<div class="article-hero"><p class="eyebrow">Reuse and rights</p><h1>MIT license</h1><p class="lede">Use, modify and share the original project materials, including commercially. Preserve the copyright and permission notices.</p></div><article class="document-content"><section class="doc-section"><h2>What this covers</h2><p>Unless an individual file states otherwise, original project code, documentation, schemas, research records and website assets are made available under MIT, to the extent contributors hold rights in them. Third-party material keeps its own license. Scholarly citation is encouraged and adds no conditions to MIT.</p><p><a href="LICENSE" download>Download the license text</a> · SPDX identifier: <code>MIT</code></p></section><section class="doc-section"><h2>Full license text</h2><pre class="license-copy">'+e(read('LICENSE'))+'</pre></section></article>'
 return {**product,'license.html':page('MIT license',license_body,'license.html'),'resources.html':page('Share and reuse',read('templates/resources.html'),'resources.html'),'sitemap.xml':seo.sitemap(),**discovery(),'review.html':page('Ten cumulative reviews',review_body,'review.html'),'guide.html':page('Research guide',guide_body,'guide.html'),'ux-review.html':page('Twenty-step UX review',ux_body,'ux-review.html')}
def discovery():
 paths=['README.md','AI_CHALLENGE.md','docs/ten-step-review.md','METHODOLOGY.md','SAFETY.md','CONTRIBUTING.md','docs/model-methodology.md','docs/research-protocol.md','docs/governance.md','challenge.json','research/index.json','research/models/EDC-M-0001.json','research/hypotheses/EDC-H-0001.json','research/experiments/EDC-E-0001.json','research/literature/sources.json','docs/state-0002-review.md','docs/allocation-protocol.md','docs/allocation-pilot.md','docs/research-states.md','research/process/index.json','research/process/transition.json','research/process/sources.json','research/process/graph.json','research/protocols/EDC-P-0001.json','research/evaluations/EDC-V-0001.json','research/decisions/EDC-D-0001.json','research/states/index.json','LICENSE']
 ...
 intro+='State 0001 preserves the original implementation. State 0002 adds a separate, untested proposal for choosing research actions under resource limits. Its evaluation is not registered or run. No advantage for structured human-AI research is established.\n\n'
 index=intro+'## Start here\n\n'
 for path,label,note in [('next.html','What should we learn next?','State 0002 review, planning checklist and preserved history.'),('AI_CHALLENGE.md','AI instructions','Reading order, evidence rules and first deliverable.'),('challenge.json','Challenge record','Mission, current phase and missing evidence.'),('review.html','Framing review','Ten cumulative passes, not independent peer review.'),('guide.html','Research guide','Method, safety, sources and review process.'),('research/index.json','Research index','Permanent identifiers and current statuses.'),('discovery.json','File manifest','Machine-readable paths and media types.'),('llms-full.txt','Combined reading pack','Generated from canonical documents and records.')]:
  index+=f'- [{label}]({seo.SITE+path}): {note}\n'
 index+='\n## Research records\n\n'
 for path in [p for p in paths if p.endswith('.json')]:index+=f'- [{path}]({seo.SITE+path})\n'
 index+='\n## Review and reuse\n\n'
 for path,label in [('contribute.html','Draft a question'),('resources.html','Downloads and citation'),('CITATION.cff','Citation metadata'),('license.html','License scope and reuse'),('LICENSE','MIT license for original project content'),('docs/ux-review.md','Interface review')]:index+=f'- [{label}]({seo.SITE+path})\n'
 ...
 for path in paths:
  ...
  elif path.endswith('.json'):content='```json\n'+content.rstrip()+'\n```\n'
  full+=f'---\n\nSource: {seo.SITE+path}\n\n'+content+'\n\n'
 manifest={'name':seo.NAME,'url':seo.SITE,'repository':REPO,'license':seo.SITE+'LICENSE','purpose':'Problem-framing research; no project results are recorded at launch. Consult canonical records for current status.','pages':[{'url':seo.canonical(p),'title':v[0]} for p,v in seo.PAGES.items()],'files':[{'url':seo.SITE+p,'media_type':'application/json' if p.endswith('.json') else 'text/markdown' if p.endswith('.md') else 'text/plain'} for p in paths],'schemas':[seo.SITE+p.relative_to(ROOT).as_posix() for p in sorted((ROOT/'schemas').glob('*.json'))]}
